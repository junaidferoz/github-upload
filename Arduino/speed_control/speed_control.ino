...
void setup() {
  ...
  lcd.begin(16,2);   //initialization of 16x2 LCD
}
...
void loop() {
  ...
  for(int i=150;i<255;i+=10){
    analogWrite(en,i);    //feeding the PWM value to the motor via analaog pin
    lcd.setCursor(0,0);
    ...
    if(i>=230){ //detection of overspeeding
        ...
        for(int j=230;j>=150;j-=10){      //reduction in speed begins
          analogWrite(en,j);
          lcd.setCursor(0,1);
          lcd.print("RPM : "+String(display_speed((float)j)));  
...
/*display_LCD(float disp){
  lcd.setCursor(0,0);
  lcd.print("RPM="+String(disp));
}*/
